...
for (i = 0; i < TAM; i++) {
    ...
    printf("Ingresa un valor entero: ", valor);
    scanf("%d", &valor);
    
}

}
// e) Encontrar el menor valor y el mayor valor almacenados en el arreglo, junto con las posiciones correspondientes.

// f) Leer un valor desde teclado y contar cuántos múltiplos de dicho valor hay en el arreglo.

// g) Leer un valor entero desde teclado y mostrar por pantalla los valores del arreglo que estén almacenados en celdas cuyo índice (posición) sea múltiplo del valor leído.

// h) Invertir el arreglo sobre sí mismo (sin usar otro arreglo auxiliar) y desplegarlo por pantalla luego de invertido.
